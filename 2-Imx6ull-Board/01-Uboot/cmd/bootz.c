...
typedef struct OBC_BLK_PARTITIONS
{
    char lable[16];
    uint32_t addr;
    uint32_t size;
    uint8_t flag;
}OBC_BLK_PARTITIONS_T;

uint8_t g_blk_cnt = 0;
OBC_BLK_PARTITIONS_T g_st_obc_blk_info[16];

void obc_parse_partitions(const void *fdt, int node_offset)
{
    int subnode_offset;
    int label_len;
    const char *label;
    ...
    const char *flag;
    int flag_len;
    OBC_BLK_PARTITIONS_T *pblkinfo = g_st_obc_blk_info;
        ...
        subnode_offset = fdt_next_subnode(fdt, subnode_offset))
    {
       ...
       if (label) {
           memcpy(pblkinfo[g_blk_cnt].lable, label, label_len);
       }
       ...
       if (reg) 
       {
           pblkinfo[g_blk_cnt].addr = fdt32_to_cpu(reg[0]); /* 32-bit address */
           pblkinfo[g_blk_cnt].size = fdt32_to_cpu(reg[1]); /* 32-bit size */
       }

       printf("lable = %s, addr = 0x%x, size = 0x%x\n", 
                            pblkinfo[g_blk_cnt].lable,
                            pblkinfo[g_blk_cnt].addr,
                            pblkinfo[g_blk_cnt].size);

       g_blk_cnt++;
    }
}

int obc_find_usdhc2_node(const void *fdt, int node_offset)
{
    ...
    for (subnode_offset = fdt_first_subnode(fdt, node_offset);
        subnode_offset >= 0;
        subnode_offset = fdt_next_subnode(fdt, subnode_offset)) {
        int found_node = obc_find_usdhc2_node(fdt, subnode_offset);
        if (found_node >= 0) {
            return found_node;
        }
    }
...
void obc_parse_device_tree(const void *fdt) {
   int node_offset;
   int partitions_node;

   /* 从根节点开始查找 &usdhc2 节点 */
   node_offset = obc_find_usdhc2_node(fdt, 0);
   if (node_offset < 0) {
   ...
   /* 解析 partitions 节点 */
   obc_parse_partitions(fdt, partitions_node);
}
...
// 生成blkdevparts参数
void obc_generate_blkdevparts(char* blkdevparts, const char* device)
{
    char temp[64];
    blkdevparts[0] = '\0';
    OBC_BLK_PARTITIONS_T *pblkinfo = g_st_obc_blk_info;
    ...
    for (int i = 0; i < g_blk_cnt; i++) {
        ...
        sprintf(temp, "%s@%s(%s)", size_str, addr_str, pblkinfo[i].lable);
...
int obc_set_bootargs(void) 
{
    char blkdevparts[512];
    const char* device = "mmcblk1";

    char bootargs[1024];
    char *env_bootargs = "console=ttymxc0,115200 root=/dev/mmcblk0p2 rootwait rw";
    int len;

    obc_generate_blkdevparts(blkdevparts, device);
    printf("blkdevparts=%s\n", blkdevparts);

    // 将当前bootargs和blkdevparts拼接
    len = snprintf(bootargs, sizeof(bootargs), "setenv bootargs %s blkdevparts=%s", env_bootargs, blkdevparts);
...
int do_bootz(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    ...
    /* 解析设备树 */
    obc_parse_device_tree(fdt_addr);
