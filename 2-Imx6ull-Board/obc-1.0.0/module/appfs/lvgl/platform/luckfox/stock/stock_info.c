...
void stock_update_display(void)
{
    ...
    for (int i = 0; i < g_stock_count && i < MAX_STOCKS; i++) {
        ...
        /* 涨跌幅超过3%时，股票名称也设置为对应颜色 */
        if (change_int > 300) {  /* 涨幅 > 3% */
            lv_obj_set_style_text_color(g_stock_widgets.name_labels[i], lv_color_hex(0xff0000), LV_PART_MAIN | LV_STATE_DEFAULT);
        } else if (change_int < -300) {  /* 跌幅 > 3% */
            lv_obj_set_style_text_color(g_stock_widgets.name_labels[i], lv_color_hex(0x00FF00), LV_PART_MAIN | LV_STATE_DEFAULT);
        } else {
            /* 其他情况保持白色 */
            lv_obj_set_style_text_color(g_stock_widgets.name_labels[i], lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
        }
